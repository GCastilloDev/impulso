...
import { Banknote, ArrowLeft, Calculator, Calendar, CheckCircle2, User, Percent } from 'lucide-react';
...
function LoanFormContent() {
  ...
  const handleCreateLoan = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClient || !selectedProduct || !amortizationData) return;
      ...
      <div className="lg:col-span-5 space-y-5">
        <form onSubmit={handleCreateLoan} className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
          ...
          <div className="pt-3">
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-black text-sm shadow-xl shadow-emerald-500/20 transition-all"
            >
